...
test_that("spiDE(polish = FALSE) is the fit and the test without the stage", {
  ...
  # the triplet p-value the cascade used is kept beside its FDR columns
  expect_true("p.niche" %in% names(results(a)))
...
test_that(".tau2Iterate reaches a geometric fixed point in a few steps and the plain map in many", {
  ...
  expect_true(is.data.frame(acc$trace) && nrow(acc$trace) == acc$iterations)
  expect_gt(plain$iterations, 8L)
  expect_lt(max(abs(log(unlist(plain$tau2)) - log(target))), 2e-2)
})

test_that(".tau2Iterate's tolerance is a distance to the limit, not a step size", {
  # a slowly contracting plain map (ratio 0.9): the last step falls below the
  # tolerance long before the sequence is within it, so a step-size rule would
  # report convergence 10x too early
  target <- c(SampleInt = 0.24, SampleCellTypeInt = 0.05)
  slow <- function(tau2) as.list(target + 0.9 * (unlist(tau2) - target))
  start <- as.list(1.5 * target)
  plain <- spiDE:::.tau2Iterate(start, slow, maxit = 30L, tol = 1e-2, accelerate = FALSE)
  # the log-step falls below 0.01 by the fifteenth step with a tenth of a
  # log unit still to go; a distance rule keeps going to the cap
  expect_false(plain$converged)
  expect_equal(plain$iterations, 30L)
  expect_gt(max(abs(log(unlist(plain$tau2)) - log(target))), 1e-2)
})
...
test_that("the polish converges both variance components, not just the first", {
  ...
  expect_lt(sum(grepl("tau2 from the converged fit", msgs)), 10L)
  expect_true(isTRUE(attr(fits(def)[[1]]@polish, "tau2")$converged))
...
test_that("a singular information at the end of the loop warns that the df was not refreshed", {
  ...
  f <- fits(pol)[[1]]
  # the df degrades to the between-sample scalar (S - 2 = 6), as the fit does
  expect_equal(unname(f@df), 6)
})
...
test_that("the dispersion is re-profiled at the coefficients the stage reports", {
  # the warm passes hold psi; after the loop one profile pass puts it at the
  # final mean, so @psi is the profile optimum at @alpha (to the optimiser's
  # tolerance), not the cold pass's value at a penalty the stage moved on from
  cl <- clustered()
  f <- fits(polishSpiDE(cl$fit, cl$spe, verbose = FALSE))[[1]]
