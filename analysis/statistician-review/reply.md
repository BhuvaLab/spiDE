...
2.  **Convenience for NB / covariates / imbalance.** The mixed model handles the NB-GLM and nuisance covariates without hand-rolling error strata. In fairness, we *tested* the imbalance concern and it did **not** materialise: at a 3-fold cell imbalance (90 vs 270 cells/sample) both your nested-between test and the random-intercept model stay calibrated (≈ 0.035–0.041, essentially unchanged from balanced), because with a substantial τ² the unequal within-sample precisions are a minor perturbation. So imbalance is *not* a differentiator at this level — your nested-fixed approach is robust to it too. The mixed model's edge here is just that it does the right thing automatically rather than requiring the correct stratum to be assembled by hand.
